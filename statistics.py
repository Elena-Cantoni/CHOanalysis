...
m_correlation = np.ndarray((((len(files['alpha'])-Contrast_detail.a_val)),5,Contrast_detail.a_val))
alpha_s = np.ndarray((Contrast_detail.a_val,),object)
hum = -1
for human in np.array(files['alpha'][Contrast_detail.a_val:]):
    hum +=1
    #print(hum)
    for a in range(0,Contrast_detail.a_val):
        alpha = files['alpha'][a]
...
min_index_curve = np.ndarray((len(files['alpha'])-Contrast_detail.a_val,),dtype=int)
ind =-1
    
for hum in np.array(files['alpha'][Contrast_detail.a_val:]):
    print(hum)
    ind +=1    
...
hum_linpar = np.ndarray((3,len(Contrast_detail.human_s)))
df_hum_linpar = pd.DataFrame(hum_linpar,columns=Contrast_detail.human_s, index = ['slope','intercept','std'])    
h = -1
for z in range(0,len(files['alpha'])-Contrast_detail.a_val):
    h +=1
...
for points in range(0,len(df_alpha['diam'])):
    #print(points)    
    mean = np.mean(df_alpha.iloc[points][Contrast_detail.a_val+1:])
    points_mean_std[points,0] = mean 
    h = 0
    m_diff = np.ndarray((len(df_alpha['diam']),len(Contrast_detail.human_s)))
    for hum in range(0,len(Contrast_detail.human_s)):
        #print(mean)
        h +=1
        diff_square = (df_alpha.iloc[points][Contrast_detail.a_val+h]-mean)**2
        m_diff[points,hum] = diff_square
    std = np.sqrt(sum(m_diff[points,:])/(len(Contrast_detail.human_s)-1))
    std_mean = std/np.sqrt(len(Contrast_detail.human_s)-1)
    points_mean_std[points,1] = std     
    

""" CHO curve which minimizes the mean observer curve """

points_diff = np.ndarray((Contrast_detail.a_val,len(df_alpha)))
n_col =-1
...
points_sum_dist = np.ndarray((Contrast_detail.a_val,1))
...
points_curvemin = minimum(df_alpha,df_points_sum_dist ,range(0,1),Contrast_detail.alpha_s)#files['alpha'][a_val:], files['alpha'])
...
plt.title('Linear fit, operation between points '+ txt[5:][:-4], fontsize=15)
...
lin_col = ['red','blue','green','yellow','pink']
colors = -1
for c_d_h in path_s[(Contrast_detail.a_val+1):]:
    colors +=1
...
ax.plot(df_alpha['diam'],df_alpha[points_curvemin[1]],'--o',c = 'firebrick',linewidth=2, label='CHO ' + points_curvemin[1]+ ' curve')
ax.errorbar(df_alpha['diam'],points_mean_std[:,0],points_mean_std[:,1],fmt ='.',ecolor='gray', elinewidth=3, capsize = 5)
#ax.set_xlim(0.6,4.1)
#ax.set_ylim(0,0.175)
ax.set_title('Averaged CD curve '+ txt[5:][:-4], fontsize=15)
